...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift (delx,dely,e1) => let val e2 = eval_prog (e1, env) in
				    case e2 of
					NoPoints => e2
				      | Point (x,y) => Point (x + delx, y + dely) 
				      | Line (m,b) => Line(m, b - dely + m * delx)
				      | LineSegment (x1,y1,x2,y2) => LineSegment(x1+delx,y1+dely,x2+delx,y2+dely) 
				      | VerticalLine x => VerticalLine (x+delx) 
				end 
(* CHANGE: Add a case for Shift expressions *)

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog e1 =
  case e1 of 
